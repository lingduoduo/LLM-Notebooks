...
# Shared tokenizer: Chinese has no spaces, so the original .lower().split() turned
# a whole passage into a single token and BM25 was nearly useless on Chinese text.
# Everything now goes through compare_retrieval.tokenize (jieba-based).
...
class ContextualKnowledgeBaseTools(KnowledgeBaseTools):
    ...
    def _build_bm25_indexes(self):
        ...
        if self.contextual_chunk_store:
            ...
            for chunk in self.contextual_chunk_store.values():
                # Tokenize for BM25 (jieba for Chinese, still fine for English)
    ...
    def _search_bm25(self, query: str, use_contextual: bool, top_k: int) -> List[ContextualSearchResult]:
        ...
        # Tokenize query (jieba for Chinese, still fine for English)
