...
from config import Config  # noqa: E402  (env setup above must run first)
Config.create_directories()

from conversation_history import ConversationHistory  # noqa: E402  (env setup above must run first)
from background_memory_processor import BackgroundMemoryProcessor  # noqa: E402  (env setup above must run first)
...
class TestBackgroundProcessorLoop(unittest.TestCase):
    ...
    def _make_processor(self):
        processor = BackgroundMemoryProcessor(
            user_id=self.user_id, provider="openai", verbose=False
        )
