...
from memory_manager import create_memory_manager, MemoryMode
...
class ConversationConfig:
    ...
    # Default to .env (MODEL_TEMPERATURE / MODEL_MAX_TOKENS); pass explicit
    # values to this dataclass to override per agent.
    temperature: float = field(default_factory=lambda: Config.MODEL_TEMPERATURE)
    max_tokens: int = field(default_factory=lambda: Config.MODEL_MAX_TOKENS)
...
class ConversationalAgent:
    ...
    def __init__(self, 
        ...
        Args:
            ...
            provider: LLM provider ('openai', 'siliconflow', 'doubao', 'openrouter')
        ...
        # Model precedence: explicit arg > MODEL_NAME in .env > provider default below
        model = model or Config.MODEL_NAME or None
        ...
        if self.provider == "openai":
            ...
            raise ValueError(f"Unsupported provider: {self.provider}. Use 'openai', 'siliconflow', 'doubao', or 'openrouter'")
    ...
    def chat(self, message: str) -> str:
        ...
        try:
            ...
            stream = self.client.chat.completions.create(
                ...
                **max_tokens_param(self.model, self.config.max_tokens),
                stream=True
            )
