...
# This is an end-to-end script: it calls a real LLM (entity extraction and
# recursive summarization) and needs OPENAI_API_KEY plus the heavy dependencies.
# Run it directly with `python test_indexing.py`. pytest skips it by default so
# the unit suite stays offline, fast and free.
pytestmark = pytest.mark.skipif(
    not os.getenv("STRUCTURED_INDEX_E2E"),
    reason="integration test; set STRUCTURED_INDEX_E2E=1 (and OPENAI_API_KEY) to run",
)
