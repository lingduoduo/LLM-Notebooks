...
# Must match the service default in config.py / main.py (--port).
# Override with SERVICE_URL=http://host:port or a CLI argument.
DEFAULT_BASE_URL = os.getenv("SERVICE_URL", "http://localhost:4240")
...
def run_demo(base_url: str = DEFAULT_BASE_URL):
    ...
    # Initialize client
    client = VectorSearchClient(base_url)
    
    # Check service status
    print("\n📊 Checking service status...")
    try:
        response = requests.get(f"{client.base_url}/", timeout=30)
        status = response.json()
        print(f"✅ Service is running at {client.base_url}")
        print(f"   - Index type: {status['index_type']}")
        print(f"   - Model: {status['model']}")
    except Exception as e:
        print(f"❌ Service is not reachable at {client.base_url}: {e}")
        print("Start it first with:  python main.py            (listens on port 4240)")
        print("Point elsewhere with: SERVICE_URL=http://host:port python test_client.py")
        print("                 or:  python test_client.py --url http://host:port")
        return
...
def run_performance_test(base_url: str = DEFAULT_BASE_URL):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Demo client for the vector search service.")
    parser.add_argument("--url", default=DEFAULT_BASE_URL,
                        help=f"Base URL of the service (default: {DEFAULT_BASE_URL}; "
                             f"also settable via the SERVICE_URL environment variable)")
    parser.add_argument("--performance", action="store_true",
                        help="Run the performance test instead of the demo")
    args = parser.parse_args()

    if args.performance:
        run_performance_test(args.url)
    else:
        run_demo(args.url)
