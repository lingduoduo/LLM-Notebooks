...
class SearchRequest(BaseModel):
    ...
    # ge=1 matters: a negative top_k used to slice results[:top_k], silently
    # returning the worst matches instead of the best.
    top_k: int = Field(10, ge=1, le=1000, description="Number of results to return (1-1000)")
...
async def root():
    ...
    <body>
        ...
        <script>
            ...
            function displayIndexStructure(info) {
                const container = document.getElementById('indexStructure');

                // /index/structure returns {statistics, structure, bm25_params};
                // the inverted index and document info live under .structure.
                ...
                html += '<pre>' + JSON.stringify(info.bm25_params, null, 2) + '</pre>';
...
async def index_document(request: IndexDocumentRequest):
    ...
    try:
        ...
        # Bad input from the caller (e.g. empty text) is a 400, not a server fault
        logger.warning(f"Rejected index request: {e}")
...
async def index_batch(request: BatchIndexRequest):
    ...
    try:
        ...
        logger.warning(f"Rejected batch index request: {e}")
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
...
if __name__ == "__main__":
    import sys
    from config import SPARSE_HOST, SPARSE_PORT

    # Precedence: command-line argument > SPARSE_PORT env var > default 4241.
    # config.py existed but was imported by nothing, so SPARSE_PORT/SPARSE_HOST
    ...
    if len(sys.argv) > 1:
        try:
            ...
            logger.warning(f"Ignoring non-numeric port argument {sys.argv[1]!r}; "
                           f"using {port}")
