...
# umap is optional: it only helps when a tree level has many nodes (see
# cluster_nodes, which skips the reduction for small levels anyway). Importing
# it at module level made the whole RAPTOR indexer unimportable when umap was
# missing -- or installed but broken, e.g. llvmlite failing to load its shared
# object on some platforms.
try:
    import umap
except Exception as _umap_error:  # ImportError, OSError from llvmlite, ...
    umap = None
    _UMAP_ERROR = _umap_error
else:
    _UMAP_ERROR = None
...
from config import RaptorConfig, llm_params
...
class RaptorIndexer:
    ...
    def summarize_text(self, text: str, max_length: int = 200) -> str:
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                **llm_params(self.config.model_name,
    ...
    def cluster_nodes(self, embeddings: np.ndarray, min_clusters: int = 2, max_clusters: int = 10) -> np.ndarray:
        ...
        # UMAP requires n_components < n_samples and n_neighbors >= 2. Upper tree
        # levels hold only a handful of nodes, so a fixed n_components=50 asked
        # UMAP to reduce e.g. 5 samples to 50 dimensions and raised. Reducing
        # fewer samples than target dimensions is meaningless anyway, so skip the
        # reduction entirely when the level is small.
        n_components = min(50, n_samples - 2)
        if umap is not None and embeddings.shape[1] > 50 and n_components >= 2:
            reducer = umap.UMAP(
            ...
            if umap is None:
                logger.warning(f"umap unavailable ({_UMAP_ERROR}); clustering the "
                               f"{embeddings.shape[1]}-dim embeddings directly. "
                               f"Install umap-learn for dimensionality reduction.")
            else:
                logger.debug(f"Skipping UMAP for {n_samples} samples "
                             f"(too few for {embeddings.shape[1]}-dim reduction)")
    ...
    def build_tree_level(self, node_ids: List[str]) -> List[str]:
        ...
        # Get embeddings for nodes (the per-cluster texts are gathered below,
        # once the cluster assignment is known)
