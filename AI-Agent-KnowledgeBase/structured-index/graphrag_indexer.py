...
from config import GraphRAGConfig, llm_params
...
class GraphRAGIndexer:
    ...
    def extract_entities_relationships(self, text: str) -> Tuple[List[Dict], List[Dict]]:
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                **llm_params(self.config.llm_model, max_tokens=1000, temperature=0.1),
            )
        ...
        for comm_id, entity_ids in communities.items():
            ...
            try:
                response = self.client.chat.completions.create(
                    ...
                    **llm_params(self.config.summarization_model,
    ...
    def multi_hop_search(self, start_entity: str, max_hops: int = 2,
        ...
        """
        Multi-hop relational retrieval: walk the relationship edges of the
        knowledge graph to answer questions like "what connects A to B" --
        relational questions that flat vector retrieval cannot express (the
        book's "multi-hop relational reasoning").

        How this differs from search(): search() only recalls isolated
        entities/communities by embedding similarity, whereas this method
        actually uses the graph structure and returns the **relationship paths**
        leading out from the starting entity.

        Args:
            start_entity: Name of the starting entity (case-insensitive,
                matched as a substring).
            max_hops: Maximum number of hops.
            relation_filter: If given, keep only paths whose final edge has this
                relationship type.
            top_k: Maximum number of paths to return.

        Returns:
            Each path looks like {"target", "target_type", "hops", "path"},
            where path is a list of {"source", "relation", "target"} steps.
        """
        # Locate the starting node by substring match on the entity name
        start_id = None
        ...
        if start_id is None or start_id not in self.graph:
            logger.warning(f"multi_hop_search: starting entity '{start_entity}' not found")
            return []

        # Breadth-first walk along the edges, collecting paths of <= max_hops
