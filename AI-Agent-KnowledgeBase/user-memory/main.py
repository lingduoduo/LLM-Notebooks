...
# Configure logging
Config.setup_logging()  # honors LOG_LEVEL / LOG_FILE from .env
...
def run_evaluation_mode(user_id: str, memory_mode: MemoryMode, verbose: bool = True, provider: Optional[str] = None, model: Optional[str] = None):
    ...
    try:
        ...
        # The first three bindings are unused on purpose: importing them loads the
        # evaluation framework's own config/models/evaluator into sys.modules so
        # that `framework` resolves them instead of this package's same-named modules.
        import config as eval_config  # noqa: F401
        import models as eval_models  # noqa: F401
        import evaluator as eval_evaluator  # noqa: F401
        ...
        if choice == "1":
            ...
            if hasattr(agent.memory_manager, 'clear_all_memories'):
                ...
                if "No previous memory" not in memory_check:
                    print("  ⚠️ Warning: Agent memory may not be fully cleared")
                else:
                    print("  ✅ Agent memory cleared successfully")
            ...
            if hasattr(processor.memory_manager, 'clear_all_memories'):
                ...
                if "No previous memory" not in memory_check:
                    print("  ⚠️ Warning: Processor memory may not be fully cleared")
                else:
                    print("  ✅ Processor memory cleared successfully")
            ...
            if processor.conversation_history:
                ...
                print("  ✅ Cleared processor conversation history")
            ...
            print("  ✅ All memory and state cleared - ready for test case")
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help="LLM provider (defaults to env PROVIDER or 'openai')"
    )
