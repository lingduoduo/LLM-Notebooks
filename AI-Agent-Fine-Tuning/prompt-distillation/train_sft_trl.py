...
# Note: trl's SFTTrainer / SFTConfig are imported lazily inside train_model(),
# so the CLI help still works when trl is not installed (e.g. reading --help offline).
...
def main():
    parser = argparse.ArgumentParser(
        description="Train the prompt distillation student model (direct answers, no prompt) with Hugging Face TRL",
    ...
    parser.add_argument(
        ...
        help="Student base model name or path (the non-thinking model used for distillation)",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of training epochs (default 1, matching the OpenAI cookbook)",
    )
    parser.add_argument(
        "--per_device_train_batch_size",
        type=int,
        default=4,
        help="Per-GPU batch size (default 4, matching the OpenAI cookbook)",
    )
    parser.add_argument(
        "--gradient_accumulation_steps",
        type=int,
        default=4,
        help="Gradient accumulation steps (default 4, matching the OpenAI cookbook)",
    )
    parser.add_argument(
        "--learning_rate",
        type=float,
        default=2e-4,
        help="Learning rate (default 2e-4, matching the OpenAI cookbook)",
    )
    parser.add_argument(
        "--max_length",
        type=int,
        default=2048,
        help="Maximum sequence length (default 2048, matching the OpenAI cookbook)",
    )
    parser.add_argument(
        "--warmup_ratio",
        type=float,
        default=0.03,
        help="Warmup ratio (default 0.03, matching the OpenAI cookbook)",
    )
    parser.add_argument(
        "--lr_scheduler_type",
        type=str,
        default="cosine_with_min_lr",
        help="Learning rate scheduler type (default cosine_with_min_lr, matching the OpenAI cookbook)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="wandb run name (auto-generated when omitted)",
    )
