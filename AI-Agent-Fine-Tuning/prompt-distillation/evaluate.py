...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate the distilled student model (no prompt, direct answer) on language classification",
    ...
    parser.add_argument(
        ...
        help="Test text file (one text per line)",
    )
    parser.add_argument(
        "--ground_truth_file",
        type=str,
        default=None,
        help="Ground-truth label file (optional, one label per line). If omitted, labels are read from the training data",
    )
    parser.add_argument(
        "--train_data_file",
        type=str,
        default="./data/prompt_distillation_lang.jsonl",
        help="Training data file; teacher labels are extracted from it as the reference when ground_truth_file is not given",
    )
    parser.add_argument(
        "--output_file",
        type=str,
