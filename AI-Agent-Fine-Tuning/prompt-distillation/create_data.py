...
# Note: vllm / SamplingParams are imported lazily inside generate_distillation_data(),
# so the CLI help still works when vllm is not installed (e.g. reading --help offline).
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate prompt distillation training data with a teacher model (long prompt + thinking)",
    ...
    parser.add_argument(
        ...
        help="Path to the input text file (one text to classify per line)",
    )
    parser.add_argument(
        "--output_file",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Teacher model name (use a thinking model for higher accuracy)",
    )
    parser.add_argument(
        "--temperature",
        type=float,
        default=0.15,
        help="Sampling temperature (0.15, matching tinker)",
    )
    parser.add_argument(
        "--max_tokens",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of GPUs for tensor parallelism (2-4 recommended for a 30B model on H100)",
    )
    parser.add_argument(
        "--max_retries",
        type=int,
