...
def _validate_num_gpus(num_gpus):
    """Return num_gpus as a safe positive Python int."""
    if isinstance(num_gpus, bool):
        raise ValueError("num_gpus must be a positive integer, not bool")
...
def simulate_data_parallelism(data, num_gpus, model_fn):
    num_gpus = _validate_num_gpus(num_gpus)
    batch_size = len(data)
...
def memory_calculator(
    ...
    if hidden_dim and num_layers:
        ...
        # Fallback estimate for activations when architecture details are unknown.
        # Keep this modest: activations are per-GPU runtime memory and do not
        # automatically shrink just because FSDP shards parameters.
        activation_memory = params * precision_bytes * activation_fraction
...
def training_cost_estimator(
    ...
    if gpu_type not in gpu_specs:
        raise ValueError(f"Unknown gpu_type {gpu_type!r}. Choose from {list(gpu_specs)}")
    ...
    while verify["per_gpu_total_gb"] > spec["memory_gb"]:
        if num_gpus >= max_gpus:
            raise ValueError(
                f"Model does not fit within {spec['memory_gb']}GB even at {num_gpus} GPUs. "
                f"Per-GPU memory is still {verify['per_gpu_total_gb']:.1f}GB. "
                "Reduce batch/sequence length, use activation checkpointing, or use a larger-memory GPU."
            )
        num_gpus *= 2
        verify = memory_calculator(params_billions, sharding="fsdp", num_gpus=num_gpus)
    ...
    return {
        ...
        "per_gpu_memory_gb": verify["per_gpu_total_gb"],
        "fits_memory": verify["per_gpu_total_gb"] <= spec["memory_gb"],
    }
...
if __name__ == "__main__":
    ...
    x = np.random.randn(4, 2048)
    W = np.random.randn(2048, 2048)
    ...
    for params, tokens, gpu, n_gpus in estimates:
        ...
        print(f"  {params:>6}B {tokens:>6.1f}T {result['num_gpus']:>6} {result['wall_clock_days']:>7.0f}d {cost_str:>14}")
