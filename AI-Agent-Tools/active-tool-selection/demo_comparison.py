...
5. Send summary report via email"""
    ...
    print("   • Initial tools: 0")
    print("   • Tools after request 1: GitHub tools")
    print("   • Tools after request 2: Filesystem + GitHub")
    print("   • Tools after request 3: Analytics + Filesystem + GitHub")
    print("   • Tools after request 4: Communication + Analytics + Filesystem + GitHub")
    print()
...
def run_offline_benchmark(servers, top_k: int, scaling: bool = True) -> dict:
    ...
    rows = [
        ...
        'Tools in context': f"{strat['retrieval']['tools_in_context']:.1f}",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Active tool selection experiment: compare injecting every tool into context "
            "with retrieving tools on demand.\n"
            "As catalogs grow into the hundreds, dynamic retrieval greatly reduces context-token "
            "cost while preserving recall and reducing selection errors.\n\n"
            "Strategies:\n"
            "  all-tools  Inject every tool at once (traditional passive baseline)\n"
            "  retrieval  Retrieve and inject the top-k tools by task semantics "
            "(tool retrieval / RAG-style)\n"
            "  active     MCP-Zero-style active discovery: the model iteratively requests "
            "the required tools\n\n"
            "Offline tables (recall / token cost / scaling with catalog size) run without "
            "an API key;\n"
            "end-to-end accuracy and latency comparisons require an API key."
        ),
        epilog=(
            "Examples:\n"
            "  python demo_comparison.py --offline                 # Offline comparison only; no API\n"
            "  python demo_comparison.py --offline --num-tools 200 # Scale to 200 tools before comparing\n"
            "  python demo_comparison.py --strategy compare        # Compare all three strategies; requires API\n"
    ...
    parser.add_argument(
        ...
        default="compare",
        help="Strategy for end-to-end evaluation; compare runs all three (default: compare)",
    )
    parser.add_argument(
        "--query", type=str, default=None,
        help="Run the selected strategy for one query instead of the full benchmark (requires API)",
    )
    parser.add_argument(
        "--num-tools", type=non_negative_int, default=0, metavar="N",
        help="Expand the catalog to N tools with synthetic distractors to assess scaling; "
        "0 keeps the real catalog (default: 0)",
    )
    parser.add_argument(
        "--top-k", type=positive_int, default=config.TOP_K_TOOLS, metavar="K",
    ...
    parser.add_argument(
        ...
        help=f"Override the LLM model (default: {config.OPENAI_MODEL})",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Run only deterministic offline comparisons (recall/token cost) without API calls",
    )
    parser.add_argument(
        "--legacy-demos", action="store_true",
        help="Also run the original narrative demos (semantic routing, iterative discovery, and so on; requires API)",
    )
    return parser
...
""")
    ...
    if args.offline:
        print("\n[offline mode] Skipping all evaluations that require API access.")
    elif not _has_api_key():
        print("\n[notice] OPENAI_API_KEY was not detected; skipping end-to-end evaluation "
              "(accuracy/latency).")
        print("         Configure .env to run the end-to-end comparison, or use --offline "
              "to run only the offline portion explicitly.")
    else:
        if args.query:
    ...
    print(
        "As tool catalogs grow into the hundreds, injecting every tool into context wastes "
        "tokens and interferes with decisions.\n"
        "On-demand retrieval reframes tool selection as knowledge retrieval, keeping tool-description "
        "token costs low while preserving recall and reducing selection errors.\n\n"
