...
def build_catalog(num_tools: int = 0) -> List[ServerDefinition]:
    ...
    """
    if num_tools < 0:
        raise ValueError("num_tools cannot be negative")
...
def evaluate_offline(servers: List[ServerDefinition], top_k: int,
    ...
    details. Two strategies are directly comparable offline:
      ...
      * ``all-tools``  — inject every available tool schema. Recall reflects whether
        each task's gold tool exists in the supplied catalog, while token cost grows
        with the catalog.
    ...
    return {
        ...
        "strategies": {
            "all-tools": {
                ...
                "recall": all_tools_hits / n,
            },
            "retrieval": {
                "tools_in_context": retrieval_tools_sum / n,
