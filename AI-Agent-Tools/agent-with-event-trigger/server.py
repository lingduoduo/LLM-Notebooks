...
async def init_agent():
    ...
    logger.info(f"✅ Agent initialized with OpenAI model: {agent.model}")
...
def build_parser() -> argparse.ArgumentParser:
    """Build the CLI parser (command-line flags take precedence over env vars)."""
    parser = argparse.ArgumentParser(
        description="HTTP server for the event-driven Agent (FastAPI): exposes "
                    "/event and friends so webhook-style external callbacks are "
                    "turned into events that wake the Agent.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""Examples:
  python server.py                          # defaults (port 8000, MCP tools enabled)
  python server.py --port 9000              # custom port
  python server.py --model gpt-4.1          # pick a different OpenAI model
  python server.py --no-mcp                 # built-in tools only, skip MCP
  Then send events with the client: python client.py --mode test
""",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Port to listen on (default: AGENT_PORT env var, else 8000)",
    )
    parser.add_argument(
        "--model", default=None,
        help=f"OpenAI model to use (default: OPENAI_MODEL env var, else {DEFAULT_MODEL})",
    )
    parser.add_argument(
        "--no-mcp", action="store_true",
        help="Disable MCP tools and use only the built-in ones "
             "(equivalent to ENABLE_MCP_TOOLS=false)",
    )
    return parser
...
def main():
    ...
    # CLI flags override env vars: init_agent() reads these during lifespan startup
    if args.model:
