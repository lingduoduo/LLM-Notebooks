...
# Load OPENAI_API_KEY (and friends) from the .env next to this file, then from
# any .env in the working directory. Real environment variables always win.
load_dotenv(Path(__file__).with_name(".env"))
load_dotenv()



# OpenAI defaults. Override either one from the environment (or a .env file)
# without touching the code.
#
# gpt-5.2 is the default because this agent drives everything through function
# tools, and the newest gpt-5.6-* models reject function tools on
# /v1/chat/completions unless reasoning is switched off entirely.
DEFAULT_MODEL = "gpt-5.2"
DEFAULT_BASE_URL = "https://api.openai.com/v1"

# Models that cannot combine function tools with reasoning on chat completions.
_NO_CHAT_TOOLS_PREFIXES = ("gpt-5.6",)


def is_reasoning_model(model) -> bool:
    """True for OpenAI reasoning models (the GPT-5 family and the o-series).

    They differ from the older chat models in two ways that matter here: they
    take ``max_completion_tokens`` rather than ``max_tokens``, and several of
    them reject any ``temperature`` other than the default.
    """
    m = str(model or "").lower().split("/")[-1]
    return m.startswith(("gpt-5", "o1", "o3", "o4"))


def supports_chat_tools(model) -> bool:
    """False for models that refuse function tools on /v1/chat/completions."""
    m = str(model or "").lower().split("/")[-1]
    return not m.startswith(_NO_CHAT_TOOLS_PREFIXES)


def completion_limits(model, temperature: float, max_tokens: int) -> Dict[str, Any]:
    """Build the sampling kwargs the given model actually accepts.

    Reasoning models are inconsistent about `temperature` (gpt-5 and the
    o-series reject anything but the default, gpt-5.1/5.2 allow it), so the
    parameter is simply omitted for them and the server-side default applies.
    """
...
def resolve_api_key() -> Optional[str]:
    ...
    Returns None when it is missing, leaving the caller to emit its own error.
    """
...
class SystemHintConfig:
    ...
    # Model configuration. Reasoning models spend part of this budget on
    ...
    # perception-tools ships as a package whose modules use relative imports, so
    # it has to be launched with `-m` from its own directory rather than by
    # file path the way the other two servers are.
    mcp_perception_tools_module: str = "perception_tools.server"
    mcp_perception_tools_dir: str = "../perception-tools"
...
class MCPServerManager:
    ...
    async def connect_server(self, name: str, script_path: Optional[str] = None,
        ...
        Args:
            name: Server name
            script_path: Path to the MCP server script (mutually exclusive with module)
            module: Module to launch with `-m` instead, for servers that ship as
                a package (mutually exclusive with script_path)
            cwd: Working directory to launch the server from; required for
                module launches so the package is importable
        ...
        try:
            if module:
                if not cwd or not os.path.isdir(cwd):
                    logger.warning(f"MCP server directory not found for '{name}': {cwd}")
                    return False
                args = ["-m", module]
                origin = f"module {module} in {cwd}"
            else:
            ...
            logger.info(f"Discovering tools from MCP server '{name}' at {origin}")
...
class EventTriggeredAgent:
    ...
    def __init__(self, api_key: str, model: Optional[str] = None,
                 ...
                 verbose: bool = True, base_url: Optional[str] = None):
        """
        Initialize the event-triggered agent

        Args:
            api_key: OpenAI API key
            model: Model to use (default: OPENAI_MODEL env var, else DEFAULT_MODEL)
            config: System hint configuration
            verbose: If True, log full details
            base_url: API base URL (default: OPENAI_BASE_URL env var, else the
                OpenAI API). Point it at any OpenAI-compatible endpoint if needed.
        """
        ...
        if not supports_chat_tools(self.model):
            logger.warning(
                f"⚠️  Model '{self.model}' does not support function tools on "
                f"/v1/chat/completions, and this agent needs them for every "
                f"tool call. Use {DEFAULT_MODEL} (or gpt-4.1) instead."
            )

        logger.info(f"Event-Triggered Agent initialized with OpenAI model: {self.model}")
    ...
    async def load_mcp_tools(self):
        ...
        # Try to connect to perception-tools (launched as a module, see config)
        percept_dir = os.path.join(agent_dir, self.config.mcp_perception_tools_dir)
        percept_loaded = await self.mcp_manager.connect_server(
            "perception",
            module=self.config.mcp_perception_tools_module,
            cwd=os.path.abspath(percept_dir)
        )
        ...
        while iteration < max_iterations:
            ...
            try:
                ...
                response = self.client.chat.completions.create(
                    ...
                    tool_choice="auto",
                    **completion_limits(self.model, self.config.temperature,
