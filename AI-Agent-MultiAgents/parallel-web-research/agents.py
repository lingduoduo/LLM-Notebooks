...
class WorkerAgent:
    ...
    async def _acknowledge_terminate(self):
        """Acknowledge the terminate exactly once.

        The coordinator's expected-ack set is a snapshot taken when the winner
        settles, so any worker that was live at that moment is expected to
        answer.  Acknowledging only from the cancellation path left a gap: a
        worker that published ``not_found`` and then received the terminate went
        straight to cleanup and never answered, failing the audit for a run that
    ...
    def _absorb_pending_terminate(self):
        """Notice a terminate that arrived but was never dequeued by _signals."""
        while True:
            try:
                pending = self.sub.inbox.get_nowait()
            except asyncio.QueueEmpty:
                return
            if pending.type == "terminate":
                self._termination_reason = pending.payload.get("reason", "cascade")
                self.terminate.set()
    ...
    async def run(self):
        ...
        while assigned.type != "task_assigned":
            if assigned.type == "terminate":
                ...
                self.terminate.set()
                await self.report(TaskState.TERMINATED, f"terminated at a safe point: {self._termination_reason}")
                await self._acknowledge_terminate()
                return
            assigned = await self.sub.get()
        signal_task = asyncio.create_task(self._signals())
        try:
            await self.report(TaskState.RUNNING, "creating an isolated Chromium context")
            self.context = await self.browsers.new_context()
            page = await self.context.new_page()
            await self.report(TaskState.RUNNING, f"loading {self.site.url}")
            navigation = await self._navigate_interruptibly(page)
            if self.terminate.is_set():
                raise asyncio.CancelledError
            await self.report(TaskState.RUNNING, "reading the rendered faculty page")
            ...
            if self.terminate.is_set():
                raise asyncio.CancelledError
            await self.report(TaskState.RUNNING, "running evidence-constrained faculty extraction")
            ...
            if profile.get("found"):
                ...
                await self.report(TaskState.SUCCEEDED, "target faculty member found")
            else:
                await self.bus.send(self.id, "coordinator", "not_found", {
                    "reason": profile.get("reason", "not found"), "source": self.site.name,
                })
                await self.report(TaskState.SUCCEEDED, "target not found on the page")
            ...
            if self.terminate.is_set():
                ...
                await self.report(TaskState.FAILED, "task timed out; the isolated browser session has been closed")
            ...
            # A terminate may have been delivered after the main path finished,
            # or be sitting in the inbox unread; either way it must be answered.
            self._absorb_pending_terminate()
            if self.terminate.is_set():
                await self._acknowledge_terminate()
...
class Coordinator:
    ...
    async def _settle(self, worker_id: str, profile: dict):
        async with self._lock:
            ...
            # Snapshot: whoever was still running when the terminate went out was
            # delivered it, and therefore owes an acknowledgement.  Workers make
            # that true by acknowledging any terminate they received, including
            # one that lands after they finished on their own.
