...
def parse_args():
    p = argparse.ArgumentParser(description="Experiment 10-4: search for a faculty member in parallel across N isolated real browser sessions")
    p.add_argument("--target", default=TARGET, help="name of the faculty member to look for")
    p.add_argument("--sites-json", help="JSON array of websites; each item has name/college/url")
    p.add_argument("--agents", type=int, default=len(DEFAULT_SITES), help="use the first N websites/Agents")
    p.add_argument("--timeout", type=float, default=120, help="per-site timeout in seconds")
    p.add_argument("--headed", action="store_true", help="show every real browser page")
    p.add_argument("--quiet", action="store_true", help="do not print individual bus messages")
    p.add_argument("--no-compare", action="store_true", help="skip the serial baseline (by default it is really run and compared)")
    p.add_argument("--output", default="artifacts/latest.json", help="path to save the measured results JSON")
    return p.parse_args()


async def main(args) -> int:
    sites = load_sites(args.sites_json, args.agents)
    print(f"target: {args.target}; real websites / isolated browser contexts: {len(sites)}")
    ...
    print(f"resource cleanup: {'PASS' if resources_ok else 'FAIL'}; "
          f"single cascade broadcast: {'PASS' if single_cascade else 'FAIL'}")
    if parallel["winner"] is None:
        print("search finished: target faculty member not found; "
              f"failure summary={parallel['failure_summary']}")
