...
def judge_position_swapped(judge: dict) -> bool:
    """Whether the judge evidence really is a complete position-swapped review.

    Counting two judgments per pair is not enough: the two repeats have to
    place the arms in opposite positions, or the repeat controls for nothing.
    This gate therefore compares the recorded ``shown_order`` values.
    """
    pairs = judge.get("pairs", [])
    if not (judge.get("paired_n") == 30
    ...
    for pair in pairs:
        ...
        if len(judgments) != 2:
            return False
        first, second = (list(item.get("shown_order") or []) for item in judgments)
        if len(first) != 2 or sorted(first) != ["skill", "transfer"]:
            return False
...
def main() -> int:
    ...
    gates = {
        ...
        "blind_quality_judge_position_swapped": judge_position_swapped(judge),
