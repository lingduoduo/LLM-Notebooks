...
from provenance import check_artifacts, check_runtime_sources, load_declared_drift
...
def test_official_manifest_binds_artifacts_and_runtime_sources():
    ...
    assert not check_artifacts(manifest, RUN)
    # The recorded source hashes describe the code as it ran.  Sources that have
    # legitimately changed since are declared in validation/source_drift.json;
    # anything else must still match, so undeclared drift fails here.
    run_key = RUN.relative_to(ROOT).as_posix()
    sources = check_runtime_sources(manifest, ROOT, load_declared_drift(ROOT, run_key))
    assert not sources["missing"], sources["missing"]
    assert not sources["undeclared_drift"], sources["undeclared_drift"]


def test_declared_source_drift_is_not_stale():
    """A declaration must describe drift that is really there."""
    manifest = json.loads((RUN / "manifest.json").read_text(encoding="utf-8"))
    run_key = RUN.relative_to(ROOT).as_posix()
    declared = load_declared_drift(ROOT, run_key)
    assert declared["reason"], "declared drift must record why the sources changed"
    sources = check_runtime_sources(manifest, ROOT, declared)
    assert not sources["stale_declarations"], sources["stale_declarations"]
